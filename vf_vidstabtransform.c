...
typedef struct TransformContext
{
    ...
    char* matrix;               // name of lens matrix file
    int tripod;
    int debug;
} TransformContext;
...
static const AVOption vidstabtransform_options[] =
{
    ...
    {
        "matrix",    "set path to lens transformation (debarrel) matrix", OFFSET(matrix),
        AV_OPT_TYPE_STRING, {.str = DEFAULT_INPUT_NAME}, .flags = FLAGS
    },
    {
...
static int config_input(AVFilterLink* inlink)
{
    ...
    av_log(ctx, AV_LOG_INFO, "    matrix    = %s\n", tc->matrix);
